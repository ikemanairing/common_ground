...
const Q7_TITLE_TEXT = "오늘 대화에서 기억하고 싶은\n한 문장은?";
const Q7_SUBTITLE_TEXT = "나중에 다시 떠올리고 싶은 말을 짧게 적어주세요.";

export default function StepQ7Screen({
  answer,
  errorMessage,
  onChangeAnswer,
  onBack,
  ...
  const {
    ...
    titleText: Q7_TITLE_TEXT,
    subtitleText: Q7_SUBTITLE_TEXT,
  });

  return (
    <section className="m-0 flex min-h-screen flex-col items-center justify-center bg-gradient-to-b from-[#fffdf2] via-background-light to-[#fff9e7] p-0 font-display text-text-main dark:bg-background-dark dark:text-slate-100">
      <div className="relative mx-auto flex h-full min-h-screen w-full max-w-md flex-col overflow-hidden bg-background-light shadow-2xl dark:bg-background-dark">
        ...
        <div className="relative z-10 flex w-full flex-col items-center justify-center px-6 pb-6 pt-2">
          <div className="mb-2 flex w-full items-center justify-between">
            <span className="inline-flex items-center gap-1 rounded-full bg-[#fff3c8] px-2.5 py-1 text-sm font-semibold text-[#9c7b14]">
          ...
          <div className="h-2 w-full overflow-hidden rounded-full bg-[#eee5c8] dark:bg-gray-700">
            <div className="h-full w-[87.5%] rounded-full bg-gradient-to-r from-[#e0be4d] to-[#be9f33] transition-all duration-300 ease-out" />
          </div>
        </div>
        ...
        <main className="no-scrollbar relative z-10 flex flex-1 flex-col overflow-y-auto px-6 pb-6">
          <div className="flex flex-1 flex-col items-center justify-center py-8">
            <div className="w-full">
              <span
                className={[
                  "mb-4 inline-block rounded-full bg-[#fff3c8] px-3 py-1 text-xs font-bold uppercase tracking-wide text-[#9c7b14] dark:bg-primary/20",
                  badgeAnimationClass,
                ]
                  .filter(Boolean)
                  .join(" ")}
              ...
              >
                <div className="relative">
                  <textarea
                    ...
                    className="form-textarea min-h-[120px] w-full resize-none rounded-2xl border border-[#e8dcae] bg-[#fffef9] p-5 text-lg font-normal leading-relaxed text-text-main shadow-sm transition-all placeholder:text-[#9d8a52]/70 focus:border-[#e0be4d] focus:ring-2 focus:ring-[#e0be4d]/20 dark:border-gray-700 dark:bg-surface-dark dark:text-slate-100 dark:placeholder:text-slate-500"
                    placeholder="예: 완벽하지 않아도 진심이면 충분하다는 말."
                  />
        ...
        <div className="sticky bottom-0 z-10 flex w-full bg-background-light/95 px-6 py-6 backdrop-blur-sm dark:bg-background-dark">
          <div className="w-full">
            ...
            <button
              ...
              onClick={onNext}
              className={[
                "flex h-14 w-full cursor-pointer items-center justify-center overflow-hidden rounded-xl bg-gradient-to-r from-[#e0be4d] to-[#be9f33] text-lg font-bold leading-normal tracking-[0.015em] text-white shadow-lg shadow-[#e0be4d]/30 transition-all hover:brightness-105 active:scale-[0.98]",
